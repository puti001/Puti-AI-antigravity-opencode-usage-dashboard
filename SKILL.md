...
## ⚠️ API 限制 (Known Limitations)

| 資料 | 狀態 | 說明 |
|------|------|------|
| Gemini Five Hour | ✅ | `GetUserStatus` gRPC，Language Server 本地 port |
| Gemini Weekly | ❌ | 在 `CloudCodeService.retrieveUserQuotaSummary`，無公開 HTTP endpoint |
| Claude / GPT | ❌ | `GetUserStatus` 回傳 `remainingFraction: None` |
